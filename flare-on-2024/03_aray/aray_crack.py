...
conditions = '''
    ...
    uint8(77) % 24 < 24 and
    uint8(77) > 5 and
    uint8(78) & 128 == 0 and inside condition
    uint8(78) % 13 < 13 and
    uint8(78) < 141 and
    uint8(78) > 24 and
    uint8(79) & 128 == 0 and inside condition
    uint8(79) < 146 and
    uint8(79) % 24 < 24 and
    uint8(79) > 31 and
    uint8(80) & 128 == 0 and
...
def crc32_bf(trg):
    for i in range(0x20, 0x7f):
        for j in range(0x20, 0x7f):
            if zlib.crc32(f'{chr(i)}{chr(j)}'.encode('utf-8')) == trg:
                return i, j

    raise Exception(f'Cannot find a CRC32 checksum for: 0x{trg:x}' )
...
def md5_bf(trg):
    for i in range(0x20, 0x7f):
        for j in range(0x20, 0x7f):
            if hashlib.md5(f'{chr(i)}{chr(j)}'.encode('utf-8')).hexdigest() == trg:
                return i, j

    raise Exception(f'Cannot find an MD5 checksum for: 0x{trg:x}' )
...
def sha256_bf(trg):
    for i in range(0x20, 0x7f):
        for j in range(0x20, 0x7f):
            if hashlib.sha256(f'{chr(i)}{chr(j)}'.encode('utf-8')).hexdigest() == trg:
                return i, j

    raise Exception(f'Cannot find a SHA256 checksum for: 0x{trg:x}' )


# ----------------------------------------------------------------------------------------
if __name__ == "__main__":
    print('[+] aray crack started.')
    
    smt = z3.Solver()
    flag = [z3.BitVec('f_%d' % i, 16) for i in range(85)]

    # Limit each variable to a printable ASCII.
    for f in flag:
        smt.add(z3.And(f >= 0x20, f <= 0x7e))

    # Iterate over each condition.
    for cond in conditions.splitlines():
        cond = cond.rstrip(' and')
        if not cond:
            continue

        # Now parse each condition and make a constraint out of it.
        if match := re.match(r'\s+filesize == 85', cond):
            pass
        elif match := re.match(r'\s+filesize \^ uint8\((\d*)\) != (\d*)', cond):
            smt.add(85 ^ flag[int(match.group(1))] != int(match.group(2)))

        elif match := re.match(r'\s+hash.crc32\((\d*), 2\) == 0x([0-9a-f]*)', cond):
            idx = int(match.group(1))
            chk = int(match.group(2), 16)
            a, b = crc32_bf(chk)

            smt.add(flag[idx] == a)
            smt.add(flag[idx + 1] == b)

        elif match := re.match(r'\s+hash.md5\((\d*), 2\) == "([0-9a-f]*)"', cond):
            idx = int(match.group(1))
            chk = match.group(2)
            a, b = md5_bf(chk)
        ...
        elif match := re.match(r'\s+hash.md5\((\d*), filesize\) == "([0-9a-f]*)"', cond):
            pass

        elif match := re.match(r'\s+hash.sha256\((\d*), 2\) == "([0-9a-f]*)"', cond):
            idx = int(match.group(1))
            chk = match.group(2)
            a, b = sha256_bf(chk)
        ...
        elif match := re.match(r'\s+uint32\((\d*)\) ([\+\-\^]) (\d*) == (\d*)', cond):
            num = (flag[int(match.group(1))] |
                  (flag[int(match.group(1)) + 1] << 8) |
                  (flag[int(match.group(1)) + 2] << 16) |
                  (flag[int(match.group(1)) + 3] << 24))

            if match.group(2) == '+':
                # Example: uint32(28) - 419186860 == 959764852
                # 
                # Adding the full 32-bit at once doesn't work:
                #   `smt.add(num + int(match.group(3)) == int(match.group(4)))`
                #
                # Instead, we add them byte-by-byte.
                diff = int(match.group(4)) - int(match.group(3)) 

                smt.add(flag[int(match.group(1))]   == diff & 0xff)
                smt.add(flag[int(match.group(1))+1] == (diff >> 8)  & 0xff)
                smt.add(flag[int(match.group(1))+2] == (diff >> 16) & 0xff)
                smt.add(flag[int(match.group(1))+3] == (diff >> 24) & 0xff)
                ...
                smt.add(flag[int(match.group(1))]   ^ ((int(match.group(3))     )  & 0xff) == int(match.group(4)) & 0xFF)
                smt.add(flag[int(match.group(1))+1] ^ ((int(match.group(3)) >> 8)  & 0xff) == (int(match.group(4)) >> 8) & 0xFF)
                smt.add(flag[int(match.group(1))+2] ^ ((int(match.group(3)) >> 16) & 0xff) == (int(match.group(4)) >> 16) & 0xFF)
                smt.add(flag[int(match.group(1))+3] ^ ((int(match.group(3)) >> 24) & 0xff) == (int(match.group(4)) >> 24) & 0xFF)
                
            else:
                raise Exception(f'Unknown operator: {match.group(2)!r}')

      
        elif match := re.match(r'\s+uint8\((\d*)\) & (\d*) == (\d*)', cond):
            smt.add(flag[int(match.group(1))] & int(match.group(2)) == int(match.group(3)))

        elif match := re.match(r'\s+uint8\((\d*)\) < (\d*)', cond):
            smt.add(flag[int(match.group(1))] < int(match.group(2)))
            ...
            raise Exception(f'Cannot parse condition: {cond!r}')
    ...
    if smt.check() == z3.sat:
        ...
        flag_txt = ''
        for i in range(85):
            flag_txt += chr(mdl.evaluate(flag[i]).as_long())
        
        print(f'[+] Flag:', flag_txt)

 
        if hashlib.md5(flag_txt.encode('utf-8')).hexdigest() == 'b7dc94ca98aa58dabb5404541c812db2':
...
# ----------------------------------------------------------------------------------------
r"""
┌─[00:05:56]─[✗:1]─[ispo@ispo-glaptop2]─[~/ctf/flare-on-challenges/flare-on-2024/03_aray]
└──> ./aray_crack.py 
[+] aray crack started.
[+] Parsing completed.
[+] Flag: rule flareon { strings: $f = "1RuleADayK33p$Malw4r3Aw4y@flare-on.com" condition: $f }
[+] Flag is correct :D
